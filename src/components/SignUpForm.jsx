...
const SignUpForm = () => {
  ...
  const handleSignUp = (e) => {
    e.preventDefault();
    // Simulate sign up — extract name from email for display
    const name = email.split('@')[0] || 'Arena Beast';
      ...
      <form className="space-y-6" onSubmit={handleSignUp}>
        <div>
          <input
            ...
            required
            className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white placeholder:text-white/30 focus:outline-none focus:border-[#00D1FF] transition-colors"
          />
        </div>
